...
namespace QLKS.Areas.Admin.Controllers
{
    public class KhachHangController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Add(tblKhachHang tblKhachHang)
        {
            if (ModelState.IsValid)
            {
                if (db.tblKhachHangs.Find(tblKhachHang.ma_kh)==null && db.tblKhachHangs.Where(x => x.mail == tblKhachHang.mail).FirstOrDefault()==null)
                {
                    db.tblKhachHangs.Add(tblKhachHang);
                    db.SaveChanges();
