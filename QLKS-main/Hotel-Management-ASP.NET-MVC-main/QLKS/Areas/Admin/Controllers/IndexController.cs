...
namespace QLKS.Areas.Admin.Controllers
{
    public class IndexController : Controller
    {
        ...
        public ActionResult Login(tblNhanVien objUser) // done
        {
            if (ModelState.IsValid) // if any model errors have been added to ModelState
            {
        ...
        public ActionResult ChonCachDatPhong() // done
        {
            return View();
        }
        public ActionResult ListPhongDangHoatDong() // done
        {
            var list = db.tblHoaDons.Where(u=>u.ma_tinh_trang == 1).
                Include(t => t.tblNhanVien).
                Include(t => t.tblPhieuDatPhong).
                Include(t => t.tblTinhTrangHoaDon);
            return View(list.ToList());
        }
        public ActionResult DSPhongGoiDV() // done
        {
        ...
        public ActionResult TraPhong(String id) // disable
        {
            if (id == null)
            {
        ...
        public ActionResult FindHdById(int? id) // done
        {
            if(id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        public ActionResult FindHdById2(int? id) // done
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        public ActionResult DonPhongXong(int? id) // done
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        public ActionResult FindRoom() // nope
        {
            return View();
        }

        public ActionResult CaNhan() // done
        {
            tblNhanVien nv = (tblNhanVien)Session["NV"];
            if (nv != null)
            {
                nv = db.tblNhanViens.Find(nv.ma_nv);
                ViewBag.ma_chuc_vu = new SelectList(db.tblChucVus, "ma_chuc_vu", "chuc_vu", nv.ma_chuc_vu); // selectList class represents a list that lets users select one item
                return View(nv);
            }
            else
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        [HttpPost]
        public ActionResult UploadFiles() // never mind
        {
